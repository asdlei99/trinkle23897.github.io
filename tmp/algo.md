...
令$n=node_x,m=node_y$，不失一般性，令$n\ge m$。若不考虑之前的二分步骤，则时空复杂度均为$O(nmd^2)$。由于$d=O(\frac{nm}{n+m})$，因此复杂度为$O(\frac{n^3m^3}{(n+m)^2})$；特别地，当$n=m$成立时，复杂度为$O(n^4)$；当$n\gg m$时，复杂度为$O(nm^3)$。该复杂正比于输出规模，因此是理论下界。

# 为什么正方形能跑出最优解？

定理1： 在步骤3中，如果是正方形，那么所有